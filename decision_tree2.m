...
if ~exist('learners.mat','file')
    ...
    for i = 1:numLearners
        learners(end+1:end+2,:) = randomFeature();
    end
...
%     for r = 1:500
%         for c = 1:500
%             locs = [r r;c c]+offsets;
%             locs(locs<1) = 1;
%             locs(locs>500) = 500;
%             val = [temp(locs(1,1),locs(2,1)); temp(locs(1,2),locs(2,2))];
%             res(r,c) = isequal(val,selector);
%         end
%     end
...
%     pause
    ...
    entropy_decrease(end+1) = init_entropy-e;
end
entropy_decrease
